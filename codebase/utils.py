...
def sample_gaussian(m, v):
    ...
    """
    eps = torch.randn(v.shape) #add epsilon drawn from N(0,I) that is same size as v
    z = m + eps * torch.sqrt(v) # z = eps*stddev + mean -> sample
...
def log_normal(x, m, v):
    ...
    # rewrite the normal PDF in a numerically stable way: A*e^B = logA + B

    normalizer = torch.sqrt(v*2*np.pi)
    log_normalizer = -torch.log(normalizer)
    log_exps = -0.5 * torch.square(x-m) / v 

    log_probs = log_normalizer + log_exps
...
def log_normal_mixture(z, m, v):
    ...
    # in the batch
    # modify z dims to match that of the K-cluster GMM params
    z = torch.unsqueeze(z, dim=1)
    z = z.expand(-1, m.shape[1], -1)
...
def partition_dataset(dataset, batch_size, validation_split=0.1, shuffle_dataset=True):
    ...
    validation_loader = DataLoader(dataset=dataset, batch_size=batch_size, sampler=valid_sampler)

    print(f"Using {len(train_indices)} samples to train and {len(val_indices)} for validation.")
...
def get_mnist_data(device, use_test_subset=True):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=100,  # Using a weird batch size to prevent students from hard-coding
        shuffle=True)
